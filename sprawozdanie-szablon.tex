...
W PFSP rozpatrywano $n$ zadań i $m$ maszyn. Każde zadanie musiało być wykonane na każdej maszynie w tej samej ustalonej kolejności (maszyna 1 $\rightarrow$ 2 $\rightarrow \ldots \rightarrow$ m). Czas przetwarzania zadania $j$ na maszynie $i$ wynosił $p_{ij}$. Poszukiwano permutacji zadań minimalizującej \textbf{sumę czasów ukończenia wszystkich zadań na ostatniej maszynie} ($\sum_{j=1}^{J} c(x_j, M)$, tzw.\ \emph{total completion time}). Nie jest to makespan ($C_{\max}$, czyli czas zakończenia wyłącznie ostatniego zadania), lecz suma wartości $c(x_j, M)$ dla każdego zadania $j$ w kolejności permutacji.

\section{Sposób rozwiązywania zadania}
Rozwiązania były kodowane jako permutacje $n$ zadań. Ocena rozwiązania polegała na obliczeniu sumy czasów ukończenia wszystkich zadań na ostatniej maszynie ($\sum C_j$) poprzez symulację harmonogramu: dla każdej maszyny i każdego zadania (w kolejności permutacji) czas zakończenia wyznaczany był jako maksimum czasu zwolnienia maszyny i czasu zakończenia poprzedniego zadania na tej samej maszynie, powiększony o czas przetwarzania. Na końcu sumowane były czasy ukończenia na ostatniej maszynie dla wszystkich zadań. Wszystkie metaheurystyki operowały bezpośrednio na permutacjach.
...
\begin{itemize}
    ...
    \item W każdej iteracji generowana była losowa permutacja $n$ zadań i obliczana jej wartość funkcji celu ($\sum C_j$).
...
\begin{itemize}
    ...
    \item Dla każdego punktu startowego (zadania) budowana była permutacja -- w każdym kroku kolejne zadanie wstawiano na pozycję minimalizującą bieżącą wartość $\sum C_j$.
...
tai500\_20 & 7855501 & 7877822.6 & 8407645 & 7062801 & 7093089.4 & 24356.3 & \textbf{6942475} & \textbf{6977049.1} & 18033.4 \\
\bottomrule
\end{tabular}}
\end{table}
...
\textbf{Instancje średnie (tai100\_*):}
\begin{enumerate}
...
\textbf{Instancja trudna (tai500\_20):}
\begin{enumerate}
    \item \textbf{SA} -- najlepszy, avg=6977049, wyraźnie lepszy od EA o ok. 1.6\%.
    \item \textbf{EA} -- drugi, avg=7093089.
    \item \textbf{Random} -- znacznie gorszy, avg=7877823 (różnica ok. 12\% względem SA).
    \item \textbf{Greedy} -- najgorszy, best=8407645 (gorszy od SA o ponad 20\%).
\end{enumerate}

\subsection{Kluczowe obserwacje}

\textbf{EA wygrało na małych i średnich instancjach, SA na największej.} Na tai20\_* i tai100\_* EA po strojeniu okazało się lepsze od SA. Jednak na tai500\_20 SA wróciło na prowadzenie -- wynik spójny z obserwacjami sprzed strojenia i potwierdzający, że przy bardzo dużej przestrzeni poszukiwań lokalne przeszukiwanie SA skaluje się lepiej niż ewolucja na populacji. Strojenie parametrów EA (zwłaszcza per-instancja pop\_size, Px i Pm) miało decydujący wpływ na jakość wyników na pozostałych instancjach.

\textbf{Greedy okazało się najgorszą metodą} mimo że jest deterministyczne i szybkie. Ograniczenie budżetowe przy greedy (mała liczba restartów), sprawia, że algorytm ten jest nieskuteczny.
